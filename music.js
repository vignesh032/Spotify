...
// let menu = document.querySelector(".menu");
// let wrong = document.querySelector(".mark");
// let playBtn = document.querySelector("#play"); // matches your HTML
...
// // Fetch mp3 list from server
// async function fetchSongs(path) {
//   let res = await fetch(`http://127.0.0.1:3000/${path}`);
...
//     if (href && href.endsWith(".mp3")) {
//       // make full URL
//       songs.push(`http://127.0.0.1:3000/${path}/${href}`);
//     }
//   }
//   return songs;
// }
...
//   }
//   document.querySelector(".songInfo").innerText = src.split("/").pop().replaceAll("%20"," ");
...
// // Load songs into a given container
// async function loadSongs(path, containerSelector, autoplayFirst = true) {
//   let songs = await fetchSongs(path);
//   let songUl = document.querySelector(containerSelector).querySelector("ul");

//   if (!songUl) return;
//   songUl.innerHTML = "";

//   if (songs.length > 0) {
//     playMusic(songs[0], !autoplayFirst);
//   }
...
//     li.innerHTML = `<div class="li"><div class="name">${fileName}</div></div>`;
...
// // 1. Load NCS immediately into .songs
// loadSongs("ncs", ".songs");

// // 2. Load /songs/ when album1 is clicked
// document.querySelector(".coolie").addEventListener("click", () => {
//   loadSongs("songs", ".songscard");
// });
